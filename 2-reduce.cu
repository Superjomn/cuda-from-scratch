...
__global__ void reduce_naive_atomic(int* g_idata,
                                    int* g_odata,
                                    unsigned int n) {
...
__global__ void reduce_smem_1_avoid_divergent_warps(int* g_idata,
                                                    int* g_odata,
                                                    unsigned int n) {
  extern __shared__ int sdata[];
...
// Avoid bank conflict
__global__ void reduce_smem_2_avoid_bank_conflict(int* g_idata,
                                                  int* g_odata,
                                                  unsigned int n) {
  extern __shared__ int sdata[];
...
__global__ void reduce_smem_3_read_two(int* g_idata,
                                       int* g_odata,
                                       unsigned int n) {
  extern __shared__ int sdata[];
...
__global__ void reduce_smem_3_read_two_bk(int* g_idata,
                                          int* g_odata,
                                          unsigned int n) {
  extern __shared__ int sdata[];
...
// Avoid divergent warps
__global__ void reduce_smem_3_read_two1(int* g_idata,
                                        int* g_odata,
                                        unsigned int n) {
  extern __shared__ int sdata[];
...
template <int NT>
__global__ void reduce_warp_shlf_read_N(int* g_idata,
                                        int* g_odata,
                                        unsigned int n) {
  // Helps to share data between warps
  // size should be (blockDim.x / warpSize)
...
#pragma unroll
  for (int i = 0; i < NT; i++) mySum += GET_ELEM(idx + i * blockDim.x);
...
template <int NT>
__global__ void reduce_warp_shlf_read_N_atomic(int* g_idata,
                                               int* g_odata,
                                               unsigned int n) {
  // Helps to share data between warps
  // size should be (blockDim.x / warpSize)
...
int launch_reduce(int* g_idata,
  ...
  int* idata = g_idata;
  int* odata = g_odata;
  if (smem_size == 0) smem_size = block_size * sizeof(int);

  // Calculate number of blocks
  num_blocks =
      (num_blocks > 0) ? num_blocks : (n + block_size - 1) / block_size;
  ...
  if (!FLAGS_profile) cudaStreamSynchronize(stream);
  ...
  int h_out;
  NVCHECK(cudaMemcpyAsync(
      &h_out, odata, sizeof(int), cudaMemcpyDeviceToHost, stream));
...
int main(int argc, char** argv) {
  ...
  auto add_wrapped = [&](cudaStream_t stream) -> void {
    switch (FLAGS_kernel) {
      case 0:
        reduce_result = launch_reduce(input.data,
                                      output.data,
                                      FLAGS_n,
                                      block.x,
                                      reduce_smem_naive,
      ...
      case 1:
        reduce_result = launch_reduce(input.data,
                                      ...
                                      FLAGS_n,
                                      block.x,
                                      reduce_smem_1_avoid_divergent_warps,
                                      stream,
                                      0,
                                      0);
        break;
      case 2:
        reduce_result = launch_reduce(input.data,
                                      ...
                                      FLAGS_n,
                                      block.x,
                                      reduce_smem_2_avoid_bank_conflict,
                                      stream,
                                      0,
                                      0);
        break;
      ...
      case 70: {
        ...
        reduce_result = launch_reduce(input.data,
                                      ...
                                      FLAGS_n,
                                      block.x,
                                      reduce_warp_shlf_read_N_atomic<NT>,
                                      stream);
      } break;
      case 71: {
        constexpr int NT = 2;
