...
void dumpGpuProps() {
  ...
  std::cerr << "  Max block dimensions: " << xthreadlimit << " x "
            << ythreadlimit << " x " << zthreadlimit << std::endl;
  std::cerr << "  Max grid dimensions: " << xgridlimit << " x " << ygridlimit
            << " x " << zgridlimit << std::endl;
  ...
  switch (sConfig) {
    case cudaSharedMemBankSizeDefault:
      std::cerr << "  Shared memory bank size: default" << std::endl;
      break;
    case cudaSharedMemBankSizeFourByte:
      std::cerr << "  Shared memory bank size: 4 bytes" << std::endl;
      break;
    case cudaSharedMemBankSizeEightByte:
      std::cerr << "  Shared memory bank size: 8 bytes" << std::endl;
      break;
  }
  ...
  NVCHECK(
      cudaDeviceGetAttribute(&x, cudaDevAttrMaxThreadsPerMultiProcessor, dev));
  std::cerr << "  Max resident threads per multiprocessor: " << x << " = "
            << x / 32 << " warps" << std::endl;
  ...
  std::cerr << "  Global L1 cache supported: " << (x ? "yes" : "no")
            << std::endl;
  ...
  std::cerr << "  Processor clock: " << deviceProp.clockRate / 1000 / 1000
            << " MHZ" << std::endl;
  std::cerr << "  Memory clock: " << deviceProp.memoryClockRate / 1000 / 1000
            << " MHZ" << std::endl;
}
