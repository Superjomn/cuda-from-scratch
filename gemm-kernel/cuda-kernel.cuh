...
inline void check_torch_dtype(torch::Tensor tensor, torch::ScalarType expected_dtype) {
...
__global__ void gemm_naive_f32_kernel(const float* A,
                                      const float* B,
                                      const float* C,
                                      float* D,
                                      int M,
...
__global__ void gemm_naive_tiled_f32_kernel(const float* A,
                                            const float* B,
                                            const float* C,
                                            float* D,
                                            int M,
...
template <int TILE_M, int TILE_N, int TILE_K, int WMMA_M = 16, int WMMA_N = 16, int WMMA_K = 16>
class GemmTiledMmaF16 {
  // Define the WMMA fragments for matrices A, B, and the accumulator
  using FragmentA = wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major>;
  using FragmentB = wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major>;
  using FragmentAcc = wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float>;

 public:
  __forceinline__ __device__ void compute(const half* A,
                                          const half* B,
                                          const float* C,
                                          float* D,
                                          int M,
    ...
    if (c_row_start < M && c_col_start < N) {
      wmma::load_matrix_sync(acc, C + c_row_start * ldc + c_col_start, ldc, wmma::mem_row_major);
    } else {
    ...
    for (int k_tile = 0; k_tile < K; k_tile += TILE_K) {
      ...
      if (tx < TILE_K && ty < TILE_M) {
        sA[ty][tx] = (a_row < M && a_col < K) ? A[a_row * lda + a_col] : __float2half(0.0f);
      }

      // Load tile for B with boundary checks
      if (tx < TILE_N && ty < TILE_K) {
        sB[ty][tx] = (b_row < K && b_col < N) ? B[b_row * ldb + b_col] : __float2half(0.0f);
      }
    ...
    if (d_row < M && d_col < N) {
      wmma::store_matrix_sync(D + d_row * ldc + d_col, acc, ldc, wmma::mem_row_major);
    }
  }
};
...
__global__ void gemm_tiled_mma_f16_kernel(const half* A,
                                          const half* B,
                                          const float* C,
                                          float* D,
                                          int M,
...
// This simply loads the matrix A and B in 128-bit chunks, and then performs the matrix
// multiplication.
template <int TILE_M, int TILE_N, int TILE_K, typename MmaShape = MmaShape16x16x16>
class GemmTiledMmaV2F16 {
 ...
 private:
  ...
  __forceinline__ __device__ void load_C_tile(const float* C, int M, int N, int K, int ldc) {
    ...
    if (c_row_start < M && c_col_start < N) {
      wmma::load_matrix_sync(acc_, C + c_row_start * ldc + c_col_start, ldc, wmma::mem_row_major);
    } else {
 ...
 public:
  __forceinline__ __device__ void compute(const half* A,
    ...
    // This code maps the 1D warp ID to 2D coordinates (warp_m_, warp_n_)
    // within the grid of warps.
    const int warpId = (threadIdx.y * blockDim.x + threadIdx.x) / WARP_SIZE;
    const int total_warps = (blockDim.x * blockDim.y) / WARP_SIZE;

    // Number of WMMA tiles in the block's tile
    constexpr int wmma_tiles_m = TILE_M / WMMA_M;
    constexpr int wmma_tiles_n = TILE_N / WMMA_N;
    constexpr int total_wmma_tiles = wmma_tiles_m * wmma_tiles_n;

    // Shared memory for tiles of A and B (declared outside the loop)
    __shared__ half sA[TILE_M][TILE_K];
    __shared__ half sB[TILE_K][TILE_N];

    // Each warp will process multiple WMMA tiles if necessary
    for (int wmma_tile_idx = warpId; wmma_tile_idx < total_wmma_tiles;
         wmma_tile_idx += total_warps) {
      // Map linear tile index to 2D coordinates
      warp_m_ = wmma_tile_idx / wmma_tiles_n;
      warp_n_ = wmma_tile_idx % wmma_tiles_n;
      ...
      // Loop over the K dimension in tile-sized steps
      for (int k_tile = 0; k_tile < K; k_tile += TILE_K) {
        // All threads participate in loading shared memory
        if (threadIdx.x + threadIdx.y * blockDim.x < blockDim.x * blockDim.y) {
          load_A_tile(sA, A, M, K, lda, k_tile);
          load_B_tile(sB, B, K, N, ldb, k_tile);
        }
        __syncthreads();

        // Only threads in valid warps participate in WMMA operations
        if (wmma_tile_idx < total_wmma_tiles) {
#pragma unroll
          for (int k = 0; k < TILE_K; k += WMMA_K) {
            half* a_ptr = &sA[warp_m_ * WMMA_M][k];
            half* b_ptr = &sB[k][warp_n_ * WMMA_N];
      ...
      if (d_row < M && d_col < N) {
        wmma::store_matrix_sync(D + d_row * ldc + d_col, acc_, ldc, wmma::mem_row_major);
      }
    }  // End of WMMA tile loop
  }
};

// Boilerplate kernel launcher for V2
template <int TILE_M, int TILE_N, int TILE_K>
__global__ void gemm_tiled_mma_v2_f16_kernel(const half* A,
                                             const half* B,
                                             const float* C,
                                             float* D,
                                             int M,
